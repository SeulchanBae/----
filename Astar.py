...
while not open_queue.empty():
    ...
    for state in current.expand(current.depth + 1):
        if state not in closed_queue and all(state != q_item[2] for q_item in open_queue.queue):
            open_queue.put((state.f(), count, state))
